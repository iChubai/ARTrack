...
class ARTrackWithResNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Tokens嵌入层 -----------------------------
        self.word_embeddings = nn.Embedding(self.bins * self.range + 6, cfg.MODEL.HIDDEN_DIM, padding_idx=self.bins * self.range + 4, max_norm=None, norm_type=2.0)
        nn.init.kaiming_normal_(self.word_embeddings.weight.data)
        self.pos_embeddings = nn.Embedding(
            num_embeddings=5,  # 5个坐标位
        ...
        # 预测头 ------------------------------
        self.output = nn.Linear(cfg.MODEL.HIDDEN_DIM, self.bins * self.range + 6)

        self.output_bias = nn.Parameter(torch.zeros(self.bins * self.range + 6))  # 偏置项
